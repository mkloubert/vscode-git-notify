...
export abstract class LoggerBase extends Events.EventEmitter implements Logger {
    ...
    public async log(type: LogType, msg: any, tag?: string) {
        ...
        const RAISE_EVENT = await Promise.resolve(
            vscgn_helpers.toBooleanSafe(await this.onLog(CONTEXT),
                                        true),
        );
    ...
    protected normalizeTag(tag: string): string {
        tag = vscgn_helpers.normalizeString(tag, s => s.toUpperCase().trim());
        if ('' === tag) {
            tag = undefined;
        }
            ...
            try {
                ...
                for (let j = 0; j < FILTERS.length; j++) {
                    try {
                        ...
                        doLog = vscgn_helpers.toBooleanSafe(
...
NEW_CONSOLE_LOGGER.addAction((ctx) => {
    let msg = vscgn_helpers.toStringSafe(ctx.message);
    ...
    let tagPrefix = '';
    if (!vscgn_helpers.isEmptyString(ctx.tag)) {
        tagPrefix = ' :: ' + vscgn_helpers.normalizeString(ctx.tag);
    }

    msg = `[${vscgn_contracts.EXTENSION_NAME}]\n[${ctx.time.format('YYYY-MM-DD HH:mm:ss')}${typePrefix}${tagPrefix}] => ${msg}\n`;
